...
int not_main() {
    ...
    for (int i = 0; i <= 1e9; ++i) {
        ...
        float _fi = hf_math::modf(_x, &_y), //My implementation
              _fc = std::modf(_x, &_z); //<cmath>
    ...
    float _wfi = hf_math::modf(_w_x, &_w_y), //My implementation
          _wfc = std::modf(_w_x, &_w_z); //<cmath>
    std::cout << "Max error: " << _max_err << "ulp\n";
    std::cout << "Worst Offender: \n";
    std::cout << std::setprecision(300)<<"modf(" << _w_x << ", iptr" <<  ")" << '\n';
    std::cout << "===========================================================\n";
    _ieee754_f32 _iwfi(_wfi), _iwfc(_wfc);
    std::cout << "Impl:    " << std::setprecision(300) << _w_y << " + "  << _iwfi._f << '\n';
    std::cout << "<cmath>: " << std::setprecision(300) << _w_z << " + " << _iwfc._f << '\n';
    std::cout << "===========================================================\n";
