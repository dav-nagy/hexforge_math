...
inline constexpr float _math_e_f = 2.71828174591064453125f;
...
inline constexpr float _math_log2_e_f = 1.44269502162933349609f;
...
inline constexpr float _math_log10_e_f = 0.434294492006301879883f;

///The mathematical constant π (pi) as a 32-bit floating-point number.
inline constexpr float _math_pi_f = 3.14159274101257324219f;

///1/π (pi) as a 32-bit floating-point number.
inline constexpr float _math_inv_pi_f = 0.318309873342514038086f;

///1/√π (1 / The square root of pi) as a 32-bit floating-point number.
inline constexpr float _math_inv_sqrt_pi_f = 0.564189612865447998047f;

///The natural logarithm of 2 as a 32-bit floating-point number.
inline constexpr float _math_log_2_f = 0.693147182464599609375f;

///The natural logarithm of 10 as a 32-bit floating-point number.
inline constexpr float _math_log_10_f = 2.30258512496948242188f;

///√2 (The square root of 2) as a 32-bit floating-point number.
inline constexpr float _math_sqrt_2_f = 1.41421353816986083984f;
///√3 (The square root of 3) as a 32-bit floating-point number.
inline constexpr float _math_sqrt_3_f = 1.73205077648162841797f;

///1/√3 (1 / The square root of 3) as a 32-bit floating-point number.
inline constexpr float _math_inv_sqrt_3_f = 0.577350258827209472656f;
...
inline constexpr float _math_egamma_f = 0.577215671539306640625f;

///φ (Phi), the Golden Ratio, defined as (1 + sqrt(5)) / 2.
inline constexpr float _math_phi_f = 1.61803400516510009766f;
...
inline constexpr unsigned int _flt_sgn_mask = 0x80000000;
///A binary mask for the exponent bits of a 32-bit floating point number.
inline constexpr unsigned int _flt_exp_mask = 0x7f800000;
///A binary mask for the mantissa bits of a 32-bit floating-point number.
inline constexpr unsigned int _flt_mant_mask = 0x007fffff;
...
inline constexpr float _flt_max_dec = 8388607.5f;

///The distance to the next representable value after 1.0 in the 32-bit floating-point format.
inline constexpr float _flt_epsilon = 1.1920928955078125e-7f;

///Absolute epsilon - The smallest (normal) 32-bit floating-point number.
inline constexpr float _flt_a_epsilon = 1.17549435082228750797e-38f;
...
inline constexpr float _flt_sub_epsilon = 1.40129846432481707092e-45f;

///The largest representable finite number in the 32-bit floating-point format.
inline constexpr float _flt_max = 3.40282346638528859812e+38f;

///For Dekker Splitting in for 32-bit floats ((23 / 2) + 1)
inline constexpr unsigned int _flt_dk_split = 13;
